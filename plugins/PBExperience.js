...
PBExperience.getExpInternal=function(level, growth){
    if(level>100){
        if(growth==0){
            //medium/medium fast
        ...
        return PBExperience.table[growth*101+level]
    }
};
PBExperience.maximunLevel=function(){
    return PBSettings.MaximunLevel;
};
    //Gets the maximum Exp Points possible for the given growth rate.
PBExperience.getMaxExperience=function(growth){
    if(growth>=6 || growth<0){
        return -1;
    }else{
        return PBExperience.getExpInternal(PBExperience.maximunLevel(),growth);
    }
};
    //Gets the number of Exp Points needed to reach the given level with given growth rate
PBExperience.getStartExperience=function(level, growth){
    if(growth>=6 || growth<0){
        return -1;
    }else{
        if(level<0){
            ...
            var mLevel=PBExperience.maximunLevel();
            if(level>mLevel){
                level=mLevel;
            }
...
PBExperience.addExperience=function(currentExp, expGain, growth){
    if(growth>=6 || growth<0){
        return 0;
    }else{
